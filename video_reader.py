...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def read_dir(self):
        ...
        if self.zip:
            ...
            train_test_val = ['train', 'test']
            for ttv in train_test_val:
                class_folders = os.listdir(os.path.join(self.data_dir, ttv))
                self.class_folders = class_folders
                for class_folder in class_folders:
                    #print(class_folder)
                    video_folders = os.listdir(os.path.join(self.data_dir, ttv, class_folder))
                    #print(video_folders)
                    video_folders.sort()
                    ...
                    for video_folder in video_folders:
                        ...
                        if c == None:
                            continue
                        imgs = os.listdir(os.path.join(self.data_dir, ttv, class_folder, video_folder))
                        #print(imgs)
                        if len(imgs) < self.seq_len:
                            continue            
                        imgs.sort()
                        paths = [os.path.join(self.data_dir, ttv, class_folder, video_folder, img) for img in imgs]
                        paths.sort()
